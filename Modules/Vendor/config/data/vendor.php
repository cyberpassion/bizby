<?php
$pg = 'vendor';

return [

    /*
    |--------------------------------------------------------------------------
    | Filters
    |--------------------------------------------------------------------------
    */
    'vendor.list-filters' => [
        'admin' => [
            'vendor_status_filter' => 'Status/status/vendor_status-json',
        ],
        'portal' => [
            'vendor_status_filter' => 'Status/status/vendor_status-json',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | Bulk Operations
    |--------------------------------------------------------------------------
    */
    'vendor.bulk-operations' => [
        'view:detail' => 'View Detail',
        'op:remove'   => 'Delete',
        'op:restore'  => 'Restore',
    ],

    /*
    |--------------------------------------------------------------------------
    | Statuses
    |--------------------------------------------------------------------------
    */
    'vendor.statuses' => [
        '1'  => 'Active',
        '11' => 'Awaiting Approval',
        '2'  => 'Inactive',
    ],

    /*
    |--------------------------------------------------------------------------
    | Default Columns
    |--------------------------------------------------------------------------
    */
    'vendor.default-columns' => [
        'entry' => [
            'vendor_id','vendor_official_name','vendor_code',
            'vendor_person','vendor_person_phone',
            'code_attaches','expected_income','tags','status',
        ],
        'list' => [
            'vendor_id','vendor_official_name','vendor_code',
            'vendor_person','vendor_person_phone',
            'code_attaches','expected_income','tags','status',
        ],
        'detail' => [
            'vendor_id','vendor_official_name','vendor_code',
            'vendor_person','vendor_person_phone',
            'code_attaches','expected_income','tags','status',
        ],
        'report' => [
            'vendor_id','vendor_official_name',
            'vendor_person','vendor_person_phone',
            'code_attaches','expected_income','tags','status',
        ],
        'sample_export' => [
            'sno','vendor_official_name','vendor_official_address',
            'vendor_official_email','vendor_official_phone',
            'vendor_terms_and_condition','vendor_person',
            'vendor_person_designation','vendor_person_email',
            'vendor_person_phone',
        ],
        'selected_columns' => [
            'vendor_official_name',
            'vendor_person',
            'vendor_person_phone',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | Documents
    |--------------------------------------------------------------------------
    */
    'vendor.documents' => [
        'performance'    => 'Performance',
        'agreement'      => 'Agreement',
        'certificate'    => 'Certificate',
        'vendor-id-card' => 'ID Card',
    ],

    /*
    |--------------------------------------------------------------------------
    | Columns (List / Report)
    |--------------------------------------------------------------------------
    */
    'vendor.list-columns' => [
        'vendor_code',
        'name',
        'vendor_type',
        'phone',
        'state',
        'status',
    ],

    'vendor.report-columns' => [
        'vendor_code',
        'name',
        'vendor_type',
        'vendor_gstin',
        'vendor_pan',
        'phone',
        'email',
        'vendor_person',
        'vendor_person_phone',
        'state',
        'district',
        'incentive_percentage',
    ],

    /*
    |--------------------------------------------------------------------------
    | Communication Templates
    |--------------------------------------------------------------------------
    */
    'communicationTemplate-vendor' => [
        'vendor_entry_new_sms'      => 'New Vendor Entry SMS',
        'vendor_entry_new_whatsapp' => 'New Vendor Entry Whatsapp',
        'vendor_entry_new_email'    => 'New Vendor Entry Email',
    ],

    /*
    |--------------------------------------------------------------------------
    | Column Name Mapping
    |--------------------------------------------------------------------------
    */
    'columnNameMapping-vendor' => [
        'ptr'                     => 'SNo',
        'vendor_id'               => 'ID',
        'vendor_code'             => 'V/Code',
        'vendor_official_name'    => 'Name',
        'vendor_official_phone'   => 'Phone',
        'vendor_official_email'   => 'Email',
        'vendor_official_address' => 'Address',
        'vendor_person'           => 'Person',
        'vendor_person_phone'     => 'P/Phone',
        'expected_income'         => 'Exp Income',
    ],

    /*
    |--------------------------------------------------------------------------
    | Module Tables
    |--------------------------------------------------------------------------
    */
    'moduleTable-vendor' => [
        'cyp_activity',
        'cyp_advancedinfo',
        'cyp_allotment',
        'cyp_cash',
        'cyp_option',
        'cyp_upload',
        'cyp_notification',
        'cyp_message',
        'cyp_vendor',
    ],

    /*
    |--------------------------------------------------------------------------
    | Form Rules
    |--------------------------------------------------------------------------
    */
    'mandatoryFields-vendor-entry-update' => [
        'vendor_official_name',
        'vendor_official_address',
        'vendor_official_email',
        'vendor_official_phone',
        'vendor_terms_and_condition',
        'vendor_person',
        'vendor_person_designation',
        'vendor_person_email',
        'vendor_person_phone',
    ],

    'dateFields-vendor-entry-update' => [],
    'additionalFields-vendor-entry-update' => [],
    'jsonFields-vendor-entry-update' => ['region'],

    /*
    |--------------------------------------------------------------------------
    | Settings & Masters
    |--------------------------------------------------------------------------
    */
    'vendor-level' => [
        'Silver',
        'Gold',
        'Platinum',
    ],

    'interactiveEntity-vendor' => ['vendor'],

];
