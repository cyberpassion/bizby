<?php

namespace Modules\Attendance\Support;

class Res
{
    public const HOME			= 'attendance.home';
    public const SESSIONS		= 'attendance.sessions';
    public const HOLIDAYS		= 'attendance.holidays';
    public const WEEKLY_OFFS	= 'attendance.weekly-offs';
    public const CALENDAR_DAYS	= 'attendance.calendar-days';
    public const SCHEDULES		= 'attendance.schedules';
    public const ATTENDANCES	= 'attendance.attendances';
    public const REPORTS		= 'attendance.reports';
	public const SETTINGS       = 'attendance.settings';
    public const MODES          = 'attendance.modes';
    public const RULES          = 'attendance.rules';
    public const STATUSES       = 'attendance.statuses';
    public const SESSION_TYPES  = 'attendance.session-types';
}
