...
return [
    ...
    // Menu
    // -------------------------------
    "menuItem-leaveapplication" => [
        "admin"  => [],
        "portal" => [
            do_ucf($pg) => [
                'home' => "{$pg}/home",
                'Apply Leave' => "{$pg}/entry",
                'All Leaves'  => "{$pg}/list"
            ]
        ]
    ],
    ...
    // Mandatory Fields
    // -------------------------------
    "mandatoryFields-leaveapplication_entry" => [],

    // -------------------------------
    // Date Fields
    // -------------------------------
    "dateFields-leaveapplication_entry" => [],

    // -------------------------------
    // Additional Fields
    // -------------------------------
    "additionalFields-leaveapplication_entry" => [],

    // -------------------------------
    // List Filters
    // -------------------------------
    "listFilters-leaveapplication_entry" => [
        "admin" => [
    ...
    "listFilters-leaveapplication_entry_update" => [
        'admin' => [
            $pg => [
                ...
                'Download Docs' => "{$pg}/download", // simplified
                'Approve'       => ['endpoint'=>$pg,'params'=>['key'=>'op:leaveapplication/approve']],
                'Reject'        => ['endpoint'=>$pg,'params'=>['key'=>'op:leaveapplication/reject']]
            ]
        ],
        'portal' => [
            $pg => [
    ...
    // Permissions
    // -------------------------------
    "permissionAdmin-leaveapplication" => [
        'restricted' => [
            '2' => [['pg' => $pg, 'sub_pg' => 'entry'], ['pg' => $pg, 'sub_pg' => 'settings']],
            '3' => [['pg' => $pg, 'sub_pg' => 'entry'], ['pg' => $pg, 'sub_pg' => 'settings']]
        ],
        'allowed' => []
    ],

    "permissionPortal-leaveapplication" => [
        'restricted' => [],
        'allowed' => [
            ['pg' => $pg, 'sub_pg' => 'home'],
            ['pg' => $pg, 'sub_pg' => 'entry'],
    ...
    // "permissionAllowedFiltersPortal-leaveapplication" => [
    //     "entry"  => [['applicant_type'=>'{$login_type}','applicant_id'=>'{$login_id}']],
    //     "list"   => [['applicant_type'=>'{$login_type}','applicant_id'=>'{$login_id}']],
    //     "report" => [['applicant_type'=>'{$login_type}','applicant_id'=>'{$login_id}']]
    // ],
