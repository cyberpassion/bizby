...
return [
    ...
    'pgStructure-consultation' => [
        $pg => [
            'forms/form' => ['entry', 'followup-entry', 'upload', 'settings', 'report'],
            'lists/list' => ['list'],
            'views/view' => array_merge(
                ( ($documents = \v3\M\Res::get("{$pg}_document-json")) ? array_keys($documents) : [] ),
    ...
    'mandatoryOptionsBeforeUsing-consultation' => [
        'consultation-entry' => [
            [
                'table'       => \v3\C\Employee::db_table('entry'),
                'params'      => [],
                'label'       => 'Please add employee to get started',
                'routeLabel'  => 'Set Now',
                'routes'      => [
                    'php'=> get_link("employee/entry"),
    ...
    'lockedFields-consultation_entry_update' => \v3\M\Option\Module::get('consultation_form_locked_column-json'),
